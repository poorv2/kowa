;if(CloudflareApps.matchPage(CloudflareApps.installs['9pVCSb25f3jr'].URLPatterns)){(function(modules){var installedModules={};function __webpack_require__(moduleId){if(installedModules[moduleId]){return installedModules[moduleId].exports;}
...
var hash=void 0
var options=CloudflareApps.installs['9pVCSb25f3jr'].options
var product=CloudflareApps.installs['9pVCSb25f3jr'].product
...
if("9pVCSb25f3jr"==='preview'&&node.nodeName==='IFRAME'&&node.src.indexOf('https://embedded.cloudflareapps.com')!==-1){return}
...
if("9pVCSb25f3jr"==='preview'){messageIndex=previewMessageIndex}else{messageIndex=Math.floor(Math.random()*options.messages.length)}
...
if("9pVCSb25f3jr"!=='preview'&&(!shouldShow||hasSeenHash())){hideWelcomeBar()
return}
updateElementStyle()
element.innerHTML=''
...
window.CloudflareApps.installs['9pVCSb25f3jr'].scope={updateOptions:function updateOptions(nextOptions){if(nextOptions.messages.length!==options.messages.length){previewMessageIndex=nextOptions.messages.length-1}else{for(var i=0;i<nextOptions.messages.length;i++){var oldEntry=options.messages[i]
